...
@AllArgsConstructor
@Tag(name = "Clientes", description = "Controller para gestionar los clientes de la empresa (Operaciones CRUD)")
public class ClientsController {

    private final UserService userService;

    @PostMapping("/register")
    public ResponseEntity<ResponseDto> registerClient(@Valid @RequestBody UserSaveRequestDTO userSaveRequestDTO){
        return userService.register(userSaveRequestDTO);
    }

    @GetMapping("/all")
    public ResponseEntity<PageResponseDto> getAllClientActive(@RequestParam(defaultValue = "") String text,
    ...
    public ResponseEntity<ResponseDto> updateClient(@RequestBody UserSaveRequestDTO userSaveRequestDTO, @PathVariable UUID userId) {
